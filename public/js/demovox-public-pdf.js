import $ from 'jquery';
import {drawImage, drawText, PDFDocumentFactory, PDFDocumentWriter, StandardFonts,} from 'pdf-lib';
import qr from 'qr-image';
// const {printJS,} = require('print-js');

$(() => {
	window.createPdf = function ($container, title, pdfUrl, fields, qrData) {
		const xhr = new XMLHttpRequest();
		xhr.open('GET', pdfUrl, true);
		xhr.responseType = 'arraybuffer';

		xhr.onload = function () {
			if (this.status === 200) {
				// response is unsigned 8 bit integer
				if (typeof this.response !== 'object' || this.response.constructor !== ArrayBuffer) {
					console.error('Loaded PDF is invalid');
					return;
				}
				let pdfData = new Uint8Array(this.response),
					pdfBytes = editPdf(pdfData, fields, qrData);
				embedPdf($container, title, pdfBytes);
			} else {
				console.error('Could not load PDF: HTTP status ' + this.status);
				return;
			}
		};

		xhr.send();
	};

	function editPdf(pdfData, fields, qrData) {
		// pdfData should be a Uint8Array.
		const pdfDoc = PDFDocumentFactory.load(pdfData),
			[helveticaRef, helveticaFont] = pdfDoc.embedStandardFont(StandardFonts.Helvetica);

		const pages = pdfDoc.getPages(), page = pages[0];

		page.addFontDictionary('Helvetica', helveticaRef);

		$.each(
			fields, function (index, value) {
				const contentStream = pdfDoc.createContentStream(
					drawText(helveticaFont.encodeText(value.drawText), {
						x: value.x,
						y: value.y,
						size: value.size,
						font: value.font,
						colorRgb: value.color,
						rotateDegrees: value.rotate,
					})
				);
				page.addContentStreams(pdfDoc.register(contentStream));
			}
		);
		if (qrData) {
			let pngBytes = createQrPng(qrData.text, qrData.size),
				[pngImage, pngDims] = pdfDoc.embedPNG(pngBytes);
			const contentStreamImg = pdfDoc.register(
				pdfDoc.createContentStream(
					drawImage('QrPng', {
						x: qrData.x,
						y: qrData.y,
						width: pngDims.width,
						height: pngDims.height,
						rotateDegrees: qrData.rotate,
					}),
				),
			);
			page.addImageObject('QrPng', pngImage)
				.addContentStreams(contentStreamImg);

			const contentStream = pdfDoc.createContentStream(
				drawText(helveticaFont.encodeText(qrData.text), {
					x: qrData.textX,
					y: qrData.textY,
					size: qrData.textSize,
					font: qrData.textFont,
					colorRgb: qrData.textColor,
					rotateDegrees: qrData.textRotate,
				})
			);
			page.addContentStreams(pdfDoc.register(contentStream));
		}
		return PDFDocumentWriter.saveToBytes(pdfDoc);
	}

	function createQrPng(qrText, size) {
		// https://www.npmjs.com/package/qr-image
		return qr.imageSync(qrText, {
			type: 'png',
			ec_level: 'L',
			size: size
		});
	}

	function embedPdf($container, title, pdfBytes) {
		let filename = title.replace(' ', '_') + '.pdf',
			data = new Int8Array(pdfBytes),
			blob = new Blob([data], {type: "application/pdf"}),
			url = window.URL.createObjectURL(blob);

		// if ($container.find('.pdf-print').length) {
		// 	$container.find('.pdf-print').click(function () {
		// 		printPDF(blobPdfFromBase64String(data), filename);
		// 	});
		// 	$container.find('.pdf-print2').click(function () {
		// 		printPDF(blob, filename);
		// 	});
		// }

		if ($container.find('.pdf-download').length) {
			$container.find('.pdf-download').click(function () {
				if (isClientIos || isClientEdge) {
					showBlob(blob, filename);
				} else {
					saveByteArray([data], filename);
				}
			});
		}

		if ($container.find('.pdf-iframe').length) {
			$container.find('.pdf-iframe').prop('src', url).show();
		}

		// const printPDF = (blob, filename) => {
		// 	try {
		// 		isClientIe11
		// 			? window.navigator.msSaveOrOpenBlob(blob, filename)
		// 			: printJS(URL.createObjectURL(blob)); // http://printjs.crabbly.com/
		// 	} catch (e) {
		// 		console.log(e);
		// 		throw 'PDFError';
		// 	}
		// };

		const isClientEdge = /Edge/.test(navigator.userAgent); // or however you want to check it
		// const isClientIe11 = !!(window.navigator && window.navigator.msSaveOrOpenBlob); // or however you want to check it
		const isClientIos = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
		// const isClientMobileOrTablet = function () {
		// 	let check = false;
		// 	(function (a) {
		// 		if (/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series([46])0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(a) || /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(a.substr(0, 4))) check = true;
		// 	})(navigator.userAgent || navigator.vendor || window.opera);
		// 	return check;
		// };

		const showBlob = (blob, name) => {
			if (window.navigator.msSaveOrOpenBlob) { //IE 11+
				window.navigator.msSaveOrOpenBlob(blob, name);
			} else if (navigator.userAgent.match('FxiOS')) { //FF iOS
				alert("Cannot display on FF iOS");
			} else if (navigator.userAgent.match('CriOS')) { //Chrome iOS
				let reader = new FileReader();
				reader.onloadend = function () {
					window.open(reader.result);
				};
				reader.readAsDataURL(blob);
			} else if (navigator.userAgent.match(/iPad/i) || navigator.userAgent.match(/iPhone/i)) { //Safari & Opera iOS
				window.location.href = window.URL.createObjectURL(blob);
			}
		};

		const saveByteArray = (function () {
			let a = document.createElement("a");
			document.body.appendChild(a);
			a.style = "display: none";
			return function (data, name) {
				let blob = new Blob(data, {type: "octet/stream"}),
					url = window.URL.createObjectURL(blob);
				a.href = url;
				a.download = name;
				a.click();
				window.URL.revokeObjectURL(url);
			};
		}());
		// const blobPdfFromBase64String = base64String => {
		// 	const byteArray = Uint8Array.from(
		// 		atob(base64String)
		// 			.split('')
		// 			.map(char => char.charCodeAt(0))
		// 	);
		// 	return new Blob([byteArray], {type: 'application/pdf'});
		// };
	}
});